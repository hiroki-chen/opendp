...
#[doc(hidden)]
pub trait BaseGaussianDomain: MappableDomain + Default {
    type InputMetric: Metric<Distance = Self::Atom> + Default;
}
impl<T: Clone + CheckAtom> BaseGaussianDomain for AtomDomain<T> {
    type InputMetric = AbsoluteDistance<T>;
}
impl<T: Clone + CheckAtom> BaseGaussianDomain for VectorDomain<AtomDomain<T>> {
    type InputMetric = L2Distance<T>;
}

#[doc(hidden)]
pub trait GaussianMeasure<DI: BaseGaussianDomain>: Measure + Default {
...
mod tests {
    ...
    fn test_make_gaussian_mechanism() -> Fallible<()> {
        let measurement = make_base_gaussian::<_, ZeroConcentratedDivergence<_>>(
            AtomDomain::default(),
